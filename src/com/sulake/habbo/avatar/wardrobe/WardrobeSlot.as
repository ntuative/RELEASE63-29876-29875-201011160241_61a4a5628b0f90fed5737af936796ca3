package com.sulake.habbo.avatar.wardrobe
{
   ...
   public class WardrobeSlot implements IOutfit, IAvatarImageListener
   {
      ...
      private var var_311:IBitmapWrapperWindow;
      ...
      private function createView() : void
      {
         ...
         this.var_311 = this._view.findChildByName("image") as IBitmapWrapperWindow;
      }
      ...
      public function updateView() : void
      {
         ...
         if(this.var_564 && this.var_1270)
         {
            _loc5_ = this._controller.avatarRenderManager.createAvatarImage(this.figure,AvatarScaleType.SMALL,this.var_840,this);
            if(_loc5_)
            {
               _loc5_.setDirection(AvatarSetType.const_32,parseInt(FigureData.const_611));
         ...
         if(this.var_311)
         {
            if(this.var_311.bitmap)
            {
               this.var_311.bitmap.dispose();
            }
            this.var_311.bitmap = new BitmapData(this.var_311.width,this.var_311.height,true,0);
            _loc7_ = (this.var_311.width - _loc1_.width) / 2;
            _loc8_ = (this.var_311.height - _loc1_.height) / 2;
            this.var_311.bitmap.draw(_loc1_,new Matrix(1,0,0,1,_loc7_,_loc8_));
         }
         if(_loc2_)
         {
            _loc1_.dispose();
      ...
      private function eventHandler(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         {
            case "set_button":
               this.var_564 = this._controller.figureData.getFigureString();
               this.var_840 = this._controller.gender;
               this._controller.handler.saveWardrobeOutfit(this.var_1638,this);
               this.updateView();
               break;
            case "get_button":
               if(this.var_564)
               {
                  this._controller.loadAvatarInEditor(this.var_564,this.var_840,this._controller.clubMemberLevel);
               }
         }
      }
