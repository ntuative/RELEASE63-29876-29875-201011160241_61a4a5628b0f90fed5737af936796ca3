package com.sulake.habbo.avatar.wardrobe
{
   ...
   public class WardrobeModel implements ISideContentModel
   {
      ...
      private function init() : void
      {
         ...
         if(this.var_472)
         {
            for each(_loc2_ in this.var_472)
            {
               _loc2_.dispose();
               _loc2_ = null;
            }
         ...
         while(_loc1_ <= 10)
         {
            this.var_472.add(_loc1_,new WardrobeSlot(this._controller,_loc1_,this.isSlotEnabled(_loc1_)));
            _loc1_++;
         }
         if(this._view)
         {
      ...
      public function updateSlots(param1:int, param2:Array) : void
      {
         ...
         {
            _loc3_ = this.var_472.getValue(_loc4_.slotId) as WardrobeSlot;
            if(_loc3_)
            {
