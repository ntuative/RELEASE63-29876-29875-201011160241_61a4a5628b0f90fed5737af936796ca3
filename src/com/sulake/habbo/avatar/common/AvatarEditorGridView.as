package com.sulake.habbo.avatar.common
{
   ...
   public class AvatarEditorGridView
   {
      ...
      public function AvatarEditorGridView(param1:IAvatarEditorCategoryModel, param2:String, param3:IHabboWindowManager, param4:IAssetLibrary)
      {
         ...
         if(this._view != null)
         {
            this.var_585 = this._view.findChildByName("part_thumbs") as IItemGridWindow;
            this.var_476 = new Array();
            this.var_476.push(this._view.findChildByName("palette0") as IItemGridWindow);
            this.var_476.push(this._view.findChildByName("palette1") as IItemGridWindow);
         }
      }
      ...
      private function paletteEventProc(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         if(param1.type == WindowMouseEvent.const_40)
         {
            ...
            _loc7_ = 0;
            while(_loc7_ < this.const_1164)
            {
               if(this.var_476.length > _loc7_)
               {
                  _loc4_ = this.var_476[_loc7_] as IItemGridWindow;
                  _loc5_ = _loc4_.getGridItemIndex(_loc6_);
                  if(_loc5_ > -1)
                  {
                     this.var_62.selectColor(this.var_1331,_loc5_,_loc7_);
                     return;
                  }
               }
               _loc7_++;
      ...
      public function initFromList() : void
      {
         ...
         var _loc5_:* = null;
         var _loc6_:* = null;
         var _loc1_:CategoryData = this.var_62.getCategoryData(this.var_1331);
         if(!_loc1_)
         {
            return;
         }
         this.var_585.removeGridItems();
         for each(_loc2_ in _loc1_.parts)
         {
            if(_loc2_)
            {
               this.var_585.addGridItem(_loc2_.view);
         ...
         _loc3_ = 0;
         while(_loc3_ < this.const_1164)
         {
            _loc4_ = _loc1_.getPalette(_loc3_);
            _loc5_ = this.var_476[_loc3_] as IItemGridWindow;
            if(!(!_loc4_ || !_loc5_))
            {
      ...
      {
         var _loc3_:IWindow = this.var_585.getGridItemAt(param1);
         if(!_loc3_)
         {
            return;
         }
      ...
      private function partEventProc(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         if(param1.type == WindowMouseEvent.const_40)
         {
            _loc3_ = this.var_585.getGridItemIndex(param1.window);
            this.var_62.selectPart(this.var_1331,_loc3_);
         }
      }
