package com.sulake.habbo.friendlist.domain
{
   ...
   public class FriendCategory implements IDisposable
   {
      ...
      public static const const_330:int = 100;
      ...
      public function removeFriend(param1:int) : Friend
      {
         ...
         if(_loc2_ != null)
         {
            Util.remove(this.var_207,_loc2_);
            return _loc2_;
         }
         return null;
      }
      ...
      {
         return Math.ceil(this.var_207.length / const_330);
      }
      ...
      {
         if(this.var_901 >= this.getPageCount())
         {
            this.var_901 = Math.max(0,this.getPageCount() - 1);
         }
      }
      ...
      public function getFriendCount(param1:Boolean, param2:Boolean = false) : int
      {
         var _loc4_:* = null;
         var _loc3_:int = 0;
         for each(_loc4_ in this.var_207)
         {
      ...
      {
         var _loc4_:* = null;
         if(this.var_207.indexOf(param1) >= 0)
         {
            return;
         }
         ...
         {
            _loc4_ = this.var_207[_loc3_];
            if(_loc2_ < _loc4_.name.toLowerCase())
            {
               this.var_207.splice(_loc3_,0,param1);
               return;
            }
            _loc3_++;
         }
      ...
      {
         this.checkPageIndex();
         return Math.min((this.var_901 + 1) * const_330,this.var_207.length);
      }
