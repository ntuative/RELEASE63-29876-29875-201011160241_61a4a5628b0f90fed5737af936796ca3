package com.sulake.habbo.friendlist.domain
{
   ...
   public class FriendCategories
   {
      ...
      private function updateCategories(param1:Array) : void
      {
         ...
         this.findCategory(FriendCategory.const_451).received = true;
         this.findCategory(FriendCategory.const_969).received = true;
         ...
         for each(_loc3_ in this.getCategoriesNotReceived())
         {
            ...
            {
               Util.remove(this.var_88,_loc3_);
               _loc3_.dispose();
            }
         }
      }
      ...
      private function removeFriend(param1:int, param2:Boolean) : Boolean
      {
         ...
         for each(_loc4_ in this.var_88)
         {
      ...
      public function onFriendListUpdate(param1:IMessageEvent) : void
      {
         ...
         try
         {
            ...
            for each(updatedFriend in p.updatedFriends)
            {
               ...
               this.var_378.messenger.setFollowingAllowed(updatedFriend.id,updatedFriend.followingAllowed && updatedFriend.online);
               currentlyOnline = this.isFriendOnline(updatedFriend.id);
               if(currentlyOnline && !updatedFriend.online)
               {
                  this.var_378.messenger.setOnlineStatus(updatedFriend.id,updatedFriend.online);
                  this.var_378.notifications.addOfflineNotification(updatedFriend.name,updatedFriend.realName);
               }
               if(!currentlyOnline && updatedFriend.online)
               {
                  this.var_378.messenger.setOnlineStatus(updatedFriend.id,updatedFriend.online);
                  this.var_378.view.setNewMessageArrived();
                  this.var_378.notifications.addOnlineNotification(updatedFriend.name,updatedFriend.figure,updatedFriend.realName);
               }
      ...
      {
         var _loc2_:int = !!param1.online ? int(param1.categoryId) : int(FriendCategory.const_451);
