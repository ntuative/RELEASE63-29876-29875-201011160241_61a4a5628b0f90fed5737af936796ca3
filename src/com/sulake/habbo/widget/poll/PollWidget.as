package com.sulake.habbo.widget.poll
{
   ...
   public class PollWidget extends RoomWidgetBase
   {
      ...
      private function showPollOffer(param1:Event) : void
      {
         ...
         var _loc3_:PollSession = this.var_255.getValue(_loc2_) as PollSession;
      ...
      override public function registerUpdateEvents(param1:IEventDispatcher) : void
      {
         ...
         param1.addEventListener(RoomWidgetPollUpdateEvent.const_134,this.showPollOffer);
         param1.addEventListener(RoomWidgetPollUpdateEvent.const_69,this.showPollError);
         param1.addEventListener(RoomWidgetPollUpdateEvent.const_127,this.showPollContent);
      ...
      override public function unregisterUpdateEvents(param1:IEventDispatcher) : void
      {
         ...
         param1.removeEventListener(RoomWidgetPollUpdateEvent.const_134,this.showPollOffer);
         param1.removeEventListener(RoomWidgetPollUpdateEvent.const_69,this.showPollError);
         param1.removeEventListener(RoomWidgetPollUpdateEvent.const_127,this.showPollContent);
      }
      
      public function pollFinished(param1:int) : void
      {
         var _loc2_:PollSession = this.var_255.getValue(param1) as PollSession;
         if(_loc2_)
         {
            _loc2_.showThanks();
            _loc2_.dispose();
      ...
      private function showPollContent(param1:Event) : void
      {
         ...
         {
            _loc3_ = _loc2_.id;
            _loc4_ = this.var_255.getValue(_loc3_) as PollSession;
            if(_loc4_)
            {
      ...
      override public function dispose() : void
      {
         ...
         if(this.var_255)
         {
            _loc1_ = this.var_255.length;
            _loc2_ = 0;
            while(_loc2_ < _loc1_)
            {
               _loc3_ = this.var_255.getWithIndex(0) as PollSession;
               if(_loc3_)
               {
                  _loc3_.dispose();
               }
