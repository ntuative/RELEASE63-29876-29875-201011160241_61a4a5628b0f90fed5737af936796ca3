package com.sulake.habbo.roomevents.userdefinedroomevents.help
{
   ...
   public class UserDefinedRoomEventsHelp
   {
      ...
      private var var_950:IContainerButtonWindow;
      
      private var var_951:IContainerButtonWindow;
      ...
      private function initLocalizations() : void
      {
         ...
         while(this.var_85.localization.getLocalization("wiredfurni.help." + _loc1_ + ".title"))
         {
            ...
            this.var_91.push([_loc2_,_loc3_,_loc4_]);
            _loc1_++;
         }
      }
      ...
      private function localize() : void
      {
         ...
         var _loc3_:IBitmapWrapperWindow = this.find(this._window,"help_image") as IBitmapWrapperWindow;
         if(!this.var_91 || this.var_91.length == 0)
         {
            return;
         }
         if(_loc1_)
         {
            _loc1_.text = ILocalization(this.var_91[this.var_213][0]).value;
         }
         if(_loc2_)
         {
            _loc2_.text = ILocalization(this.var_91[this.var_213][2]).value;
         }
         if(_loc3_)
         {
            _loc4_ = ILocalization(this.var_91[this.var_213][1]).value;
      ...
      {
         if(this.var_213 >= this.var_91.length - 1)
         {
            this.var_950.disable();
         }
         else
         {
            this.var_950.enable();
      ...
      private function prepareWindow() : void
      {
         ...
         this.var_1469 = IWindowContainer(this.find(this._window,"help_container"));
         this.var_951 = this.var_1469.findChildByName("button_previous") as IContainerButtonWindow;
         if(this.var_951)
         {
            this.var_951.procedure = this.onButtonClick;
         }
         this.var_950 = this.var_1469.findChildByName("button_next") as IContainerButtonWindow;
         if(this.var_950)
         {
            this.var_950.procedure = this.onButtonClick;
         }
      ...
      {
         ++this.var_213;
         this.var_213 = Math.min(this.var_213,this.var_91.length - 1);
         this.openPage();
      }
      ...
      {
         --this.var_213;
         this.var_213 = Math.max(this.var_213,0);
         this.openPage();
      }
