package com.sulake.habbo.catalog.viewer.widgets
{
   ...
   public class PetsCatalogWidget extends CatalogWidget implements ICatalogWidget, IPetDataListener, IPetImageListener
   {
      ...
      private function getPurchaseParameters() : String
      {
         ...
         if(this.var_479 >= this.var_389.length)
         {
            return "";
         }
         var petColor:IPetColor = this.var_389[this.var_479] as IPetColor;
         var color:uint = petColor.rgb;
         var breed:IBreed = this.var_715[this.var_1779] as IBreed;
      ...
      public function onDropMenuEvent(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         if(param1.type == WindowEvent.const_59)
         {
            ...
            if(_loc3_ >= this.var_715.length)
            {
               return;
            }
            this.var_1779 = _loc3_;
            this.updateImage();
         }
      }
      ...
      private function onColourIndex(param1:CatalogWidgetColourIndexEvent) : void
      {
         ...
         this.var_479 = param1.index;
         if(this.var_479 < 0 || this.var_479 > this.var_389.length)
         {
            this.var_479 = 0;
         }
         this.updateImage();
      ...
      override public function init() : void
      {
         ...
         _loc2_.addEventListener(WindowEvent.const_110,this.method_10);
         ...
         if(page && false && page.viewer.catalog)
         {
            ...
            if(_loc8_ && _loc8_.avatarRenderManager && _loc8_.avatarRenderManager.petDataManager)
            {
               ...
               if(_loc9_)
               {
                  this.var_715 = _loc9_.sellableBreeds;
                  this.var_389 = _loc9_.colors;
         ...
         {
            _loc10_ = this.var_715[_loc6_] as IBreed;
         ...
         events.addEventListener(WidgetEvent.const_908,this.onApproveNameResult);
      ...
      private function updateImage() : void
      {
         ...
         if(this.var_479 >= 0 && this.var_479 < this.var_389.length)
         {
            _loc7_ = this.var_389[this.var_479] as IPetColor;
            if(_loc7_ != null)
            {
               _loc2_ = uint(_loc7_.rgb);
            }
         }
         var _loc3_:IBreed = this.var_715[this.var_1779] as IBreed;
         if(!_loc3_)
         {
            return;
         }
         var _loc4_:IAvatarImage = (page.viewer.catalog as HabboCatalog).avatarRenderManager.createPetImage(this.var_885,_loc3_.id,_loc2_,AvatarScaleType.const_51,this);
         if(_loc4_ == null)
         {
            return;
         }
         _loc4_.initActionAppends();
         _loc4_.appendAction(AvatarAction.const_292,AvatarAction.const_580);
      ...
      private function onWidgetsInitialized(param1:CatalogWidgetsInitializedEvent = null) : void
      {
         ...
         var _loc3_:* = [];
         for each(_loc4_ in this.var_389)
         {
