package com.sulake.core.runtime
{
   ...
   class Profiler extends Component implements IProfiler
   {
      ...
      {
         this.var_266 = new Dictionary(true);
         super(param1,param2,param3);
      }
      
      public function stop() : void
      {
         this.var_2613 = getTimer() - this.var_2077;
      ...
      {
         var _loc2_:ProfilerAgent = this.var_266[param1];
      ...
      override public function dispose() : void
      {
         ...
         if(!disposed)
         {
            for(_loc1_ in this.var_266)
            {
               IDisposable(this.var_266[_loc1_]).dispose();
               delete this.var_266[_loc1_];
            }
            super.dispose();
         }
      }
      ...
      public function getProfilerAgentsInArray() : Array
      {
         ...
         for(_loc3_ in this.var_266)
         {
            _loc2_ = _loc3_ as IUpdateReceiver;
            if(IDisposable(_loc3_).disposed)
            {
               IDisposable(this.var_266[_loc3_]).dispose();
               delete this.var_266[_loc3_];
            }
            else
            {
               _loc1_.push(this.var_266[_loc3_]);
            }
         }
         return _loc1_;
      }
