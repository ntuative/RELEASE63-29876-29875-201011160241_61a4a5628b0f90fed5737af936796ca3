package com.sulake.core.window.graphics
{
   ...
   public class DrawBufferAllocator implements IDisposable
   {
      ...
      public function get allocatedByteCount() : uint
      {
         ...
         var bytes:uint = 0;
         var index:uint = this.var_539.length;
         try
         {
            while(index > 0)
            {
               index--;
               bitmap = BitmapData(this.var_539[index]);
      ...
      public function dispose() : void
      {
         ...
         {
            this._disposed = true;
            _loc1_ = uint(this.var_539.length);
            while(_loc1_ > 0)
            {
               BitmapData(this.var_539.pop()).dispose();
               _loc1_--;
            }
         }
      }
      ...
      {
         var _loc2_:uint = this.var_539.indexOf(param1);
         if(_loc2_ < 0)
         {
