package com.sulake.core.communication.messages
{
   ...
   public class MessageClassManager implements IMessageClassManager
   {
      ...
      public function getMessageComposerID(param1:IMessageComposer) : int
      {
         ...
         var _loc4_:* = null;
         var _loc2_:int = -1;
         for(_loc3_ in this.var_843)
         {
            _loc4_ = this.var_843[_loc3_] as Class;
            if(param1 is _loc4_)
            {
      ...
      {
         var _loc2_:Array = this.var_676[param1];
         if(_loc2_ != null)
         {
            return _loc2_;
         }
      ...
      public function toString() : String
      {
         ...
         for(_loc2_ in this.var_843)
         {
            _loc1_ += _loc2_ + " -> " + this.var_843[_loc2_] + "\n";
         }
         _loc1_ += "Registered Message Event Classes: \n";
         for(_loc2_ in this.var_676)
         {
            _loc1_ += _loc2_ + " -> " + this.var_676[_loc2_] + "\n";
         }
         return _loc1_;
      }
      ...
      private function registerMessageEvent(param1:int, param2:Class) : Boolean
      {
         ...
         {
            _loc5_ = _loc3_.@name;
            if(this.var_676[param1] == null)
            {
               this.var_676[param1] = [param2];
            }
            else
            {
               _loc6_ = this.var_676[param1];
               _loc6_.push(param2);
            }
            return true;
         }
